...
class SpotTheRT:
    ...
    def create_lobby_request(self, lobby_name_input:str, player_pseudo_input:str) -> None:
        if not lobby_name_input in self._get_dict_game().keys():
            ...
            print("ENVOYER '6' (requête créer un lobby echouée, game deja existante) AU DEMANDEUR")
            """ ENVOYER '6' (requête créer un lobby echouée, game deja existante) AU DEMANDEUR """
            ...
            if  player_pseudo_input in selected_game._get_connected_player().keys():
                if selected_game._get_connected_player()[player_pseudo_input]._get_is_hostmaster():
                    if len(selected_game._get_connected_player()) >= 2:
                        ...
                        print("ENVOYER '84' (requête lancer une partie echouée, pas assez de joueurs) AU DEMANDEUR")
                        """ ENVOYER '84' (requête lancer une partie echouée, pas assez de joueurs) AU DEMANDEUR """
                else:
                ...
                print("ENVOYER '82' (requête lancer une partie echouée, joueur inexistant) AU DEMANDEUR")
                """ ENVOYER '82' (requête lancer une partie echouée, joueur inexistant) AU DEMANDEUR """
        else:
        ...
        if lobby_name_input in self._get_dict_game().keys():
            ...
            if  player_pseudo_input in selected_game._get_connected_player().keys():
                try:
                    ...
                    nb_round_game = selected_game._get_nb_round()
                    selected_game._set_nb_round(nb_round_game - 1)
                    player_point = selected_game._get_connected_player()[player_pseudo_input]._get_player_point()
                    selected_game.selected_game._get_connected_player()[player_pseudo_input]._set_player_point(player_point - 1)
                    ...
                    print("ENVOYER '54' (requête envoi nouvelle carte commune adversaire bon) AUX AUTRES")
                    """ ENVOYER '50' (requête verif self symbole bon) AU DEMANDEUR """
                    """ ENVOYER '54' (requête envoi nouvelle carte commune adversaire bon) AUX AUTRES """
                except ValueError:
                    print("ENVOYER '53' (requête verif self symbole echouée, mauvais symbole) AU DEMANDEUR")
                    """ ENVOYER '53' (requête verif self symbole echouée, mauvais symbole) AU DEMANDEUR """
            else:
                print("ENVOYER '52' (requête verif self symbole echouée, joueur inexistant) AU DEMANDEUR")
                """ ENVOYER '52' (requête verif self symbole echouée, joueur inexistant) AU DEMANDEUR """
        else:
            print("ENVOYER '51' (requête verif self symbole echouée, partie inexistante) AU DEMANDEUR")
            """ ENVOYER '51' (requête verif self symbole echouée, partie inexistante) AU DEMANDEUR """